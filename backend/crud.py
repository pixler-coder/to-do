...
def get_list_by_name(db: Session, name: str):
    ...
    return (
        ...
        .filter(func.lower(models.List.name) == name.lower())
        .first()
    )
...
def update_list(db: Session, list_id: int, list_schema: schemas.ListUpdate):
    ...
    if not db_list:
        return None
    db_list.name = list_schema.name.strip()  # type: ignore[assignment]
    try:
        db.commit()
        db.refresh(db_list)
    except Exception:
...
def get_tasks(
    ...
    query = query.order_by(
        asc(models.Task.is_completed), asc(models.Task.due_date), asc(models.Task.id)
    )
...
def create_task(db: Session, task_schema: schemas.TaskCreate):
    db_task = models.Task(
        ...
        description=(
            task_schema.description.strip() if task_schema.description else None
        ),
